...
import eu.mihosoft.freerouting.datastructures.UndoableObjects.Storable;
...
class Structure extends ScopeKeyword
{
    ...
    public boolean read_scope(ReadScopeParameter p_par)
    {
        ...
        for (;;)
        {
            ...
            if (prev_token == OPEN_BRACKET)
            {
                ...
                else if (next_token == Keyword.SNAP_ANGLE)
                {
                    ...
                    eu.mihosoft.freerouting.board.AngleRestriction snap_angle = read_snap_angle(p_par.scanner);
                    if (snap_angle != null)
                    {
        ...
        while (it.hasNext())
        {
            ...
            if (curr_layer.no >= 0)
            {
                ...
                {
                    clearance_class_no = curr_net.get_class().default_item_clearance_classes.get(eu.mihosoft.freerouting.rules.DefaultItemClearanceClasses.ItemClass.AREA);
                }
    ...
    public static void write_scope(WriteScopeParameter p_par) throws java.io.IOException
    {
        ...
        for (;;)
        {
            ...
            if (curr_ob instanceof eu.mihosoft.freerouting.board.BoardOutline)
            {
                break;
            }
        }
        ...
        for (;;)
        {
            ...
            if (!(curr_ob instanceof eu.mihosoft.freerouting.board.ObstacleArea))
            {
                continue;
            }
            eu.mihosoft.freerouting.board.ObstacleArea curr_keepout = (eu.mihosoft.freerouting.board.ObstacleArea) curr_ob;
            if (curr_keepout.get_component_no() != 0)
            {
                // keepouts belonging to a component are not written individually.
                continue;
            }
            if (curr_keepout instanceof eu.mihosoft.freerouting.board.ConductionArea)
            {
        ...
        for (;;)
        {
            ...
            if (!(curr_ob instanceof eu.mihosoft.freerouting.board.ConductionArea))
            {
                continue;
            }
            eu.mihosoft.freerouting.board.ConductionArea curr_area = (eu.mihosoft.freerouting.board.ConductionArea) curr_ob;
            if (p_par.board.layer_structure.arr[curr_area.get_layer()].is_signal)
            {
                // These conduction areas are written in the wiring scope.
                continue;
            }
            Plane.write_scope(p_par, (eu.mihosoft.freerouting.board.ConductionArea) curr_ob);
        }
        p_par.file.end_scope();
    }
    ...
    private static void write_via_padstacks(eu.mihosoft.freerouting.library.BoardLibrary p_library, IndentFileWriter p_file,
                                            IdentifierType p_identifier_type) throws java.io.IOException
    {
        ...
        {
            eu.mihosoft.freerouting.library.Padstack curr_padstack = p_library.get_via_padstack(i);
    ...
    private static void write_control_scope(eu.mihosoft.freerouting.rules.BoardRules p_rules, IndentFileWriter p_file) throws java.io.IOException
    {
        p_file.start_scope();
    ...
    private static void write_keepout_scope(WriteScopeParameter p_par, eu.mihosoft.freerouting.board.ObstacleArea p_keepout) throws java.io.IOException
    {
        eu.mihosoft.freerouting.geometry.planar.Area keepout_area = p_keepout.get_area();
        ...
        if (keepout_area instanceof eu.mihosoft.freerouting.geometry.planar.Shape)
        {
            boundary_shape = (eu.mihosoft.freerouting.geometry.planar.Shape) keepout_area;
        ...
        p_par.file.start_scope();
        if (p_keepout instanceof eu.mihosoft.freerouting.board.ViaObstacleArea)
        {
    ...
    static eu.mihosoft.freerouting.board.AngleRestriction read_snap_angle(Scanner p_scanner)
    {
        try
        {
            Object next_token = p_scanner.next_token();
            ...
            {
                snap_angle = eu.mihosoft.freerouting.board.AngleRestriction.NINETY_DEGREE;
            }
            else if (next_token == Keyword.FORTYFIVE_DEGREE)
            {
                snap_angle = eu.mihosoft.freerouting.board.AngleRestriction.FORTYFIVE_DEGREE;
            }
            else if (next_token == Keyword.NONE)
            {
                snap_angle = eu.mihosoft.freerouting.board.AngleRestriction.NONE;
            }
            else
            {
    ...
    static void write_snap_angle(IndentFileWriter p_file, eu.mihosoft.freerouting.board.AngleRestriction p_angle_restriction) throws java.io.IOException
    {
        p_file.start_scope();
        p_file.write("snap_angle ");
        p_file.new_line();

        if (p_angle_restriction == eu.mihosoft.freerouting.board.AngleRestriction.NINETY_DEGREE)
        {
            p_file.write("ninety_degree");
        }
        else if (p_angle_restriction == eu.mihosoft.freerouting.board.AngleRestriction.FORTYFIVE_DEGREE)
        {
    ...
    private boolean create_board(ReadScopeParameter p_par, BoardConstructionInfo p_board_construction_info)
    {
        ...
        eu.mihosoft.freerouting.board.Layer[] board_layer_arr = new eu.mihosoft.freerouting.board.Layer[layer_count];
        ...
        while (5 * max_coor >= eu.mihosoft.freerouting.geometry.planar.Limits.CRIT_INT)
        {
            scale_factor /= 10;
            max_coor /= 10;
        }
        ...
        eu.mihosoft.freerouting.rules.ClearanceMatrix clearance_matrix = eu.mihosoft.freerouting.rules.ClearanceMatrix.get_default_instance(board_layer_structure, 0);
        eu.mihosoft.freerouting.rules.BoardRules board_rules = new eu.mihosoft.freerouting.rules.BoardRules(board_layer_structure, clearance_matrix);
        eu.mihosoft.freerouting.board.Communication.SpecctraParserInfo specctra_parser_info =
                new eu.mihosoft.freerouting.board.Communication.SpecctraParserInfo(p_par.string_quote, p_par.host_cad,
                p_par.host_version, p_par.constants, p_par.write_resolution, p_par.dsn_file_generated_by_host);
        eu.mihosoft.freerouting.board.Communication board_communication =
                new eu.mihosoft.freerouting.board.Communication(p_par.unit, p_par.resolution, specctra_parser_info,
    ...
    private static void insert_missing_power_planes(Collection<Layer> p_layer_info,
            NetList p_netlist, eu.mihosoft.freerouting.board.BasicBoard p_board)
    {
        Collection<eu.mihosoft.freerouting.board.ConductionArea> conduction_areas = p_board.get_conduction_areas();
        for (Layer curr_layer : p_layer_info)
        {
            ...
            for (eu.mihosoft.freerouting.board.ConductionArea curr_conduction_area : conduction_areas)
            {
            ...
            if (!conduction_area_found && !curr_layer.net_names.isEmpty())
            {
                ...
                eu.mihosoft.freerouting.rules.Net curr_net = p_board.rules.nets.get(curr_net_id.name, curr_net_id.subnet_number);
                {
                    if (curr_net == null)
                    {
    ...
    static private int append_clearance_class(BoardRules p_board_rules, String p_name)
    {
        ...
        eu.mihosoft.freerouting.rules.NetClass default_net_class = p_board_rules.get_default_net_class();
     ...
     */
    private static boolean clearance_equals(eu.mihosoft.freerouting.rules.ClearanceMatrix p_cl_matrix, int p_layer_1, int p_layer_2)
    {
    ...
    private static void insert_keepout(eu.mihosoft.freerouting.board.BasicBoard p_board, eu.mihosoft.freerouting.geometry.planar.Area p_area, int p_layer,
                                       String p_clearance_class_name, KeepoutType p_keepout_type, FixedState p_fixed_state)
    {
        ...
        {
            clearance_class_no =
                    p_board.rules.get_default_net_class().default_item_clearance_classes.get(eu.mihosoft.freerouting.rules.DefaultItemClearanceClasses.ItemClass.AREA);
        }
        else
        {
