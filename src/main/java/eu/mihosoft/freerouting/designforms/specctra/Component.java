...
public class Component extends ScopeKeyword
{
    ...
    public static void write_scope(WriteScopeParameter p_par, eu.mihosoft.freerouting.board.Component p_component)
    ...
    private static void write_pin_info(WriteScopeParameter p_par, eu.mihosoft.freerouting.board.Component p_component, int p_pin_no)
    throws java.io.IOException
    {
        if (!p_component.is_placed())
        {
    ...
    private static void write_keepout_infos(WriteScopeParameter p_par, eu.mihosoft.freerouting.board.Component p_component)
    throws java.io.IOException
    {
        if (!p_component.is_placed())
        {
        ...
        eu.mihosoft.freerouting.library.Package.Keepout[] curr_keepout_arr;
        String keepout_type;
        for (int j = 0; j < 3; ++j)
        {
            if (j == 0)
            ...
            {
                eu.mihosoft.freerouting.library.Package.Keepout curr_keepout = curr_keepout_arr[i];
                eu.mihosoft.freerouting.board.ObstacleArea curr_obstacle_area = get_keepout(p_par.board, p_component.no, curr_keepout.name);
    ...
    private static eu.mihosoft.freerouting.board.ObstacleArea get_keepout(eu.mihosoft.freerouting.board.BasicBoard p_board, int p_component_no, String p_name)
    {
        ...
        for(;;)
        {
            eu.mihosoft.freerouting.board.Item curr_item = (eu.mihosoft.freerouting.board.Item)p_board.item_list.read_object(it);
            if (curr_item == null)
            {
                break;
            }
            if (curr_item.get_component_no() == p_component_no && curr_item instanceof eu.mihosoft.freerouting.board.ObstacleArea)
            {
                eu.mihosoft.freerouting.board.ObstacleArea curr_area = (eu.mihosoft.freerouting.board.ObstacleArea) curr_item;
    ...
    private static  ComponentPlacement.ItemClearanceInfo read_item_clearance_info(Scanner p_scanner) throws java.io.IOException
    {
        ...
        if (cl_class_name == null)
        {
            ...
            // name instead; downstream lookup (clearance_matrix.get_no) returns -1 for it and the
            // caller falls back to the default SMD/PIN clearance class.
            cl_class_name = "";
        }
