...
public class SessionFile
{
    ...
    public static void write_components(BasicBoard p_board, IdentifierType p_identifier_type, CoordinateTransform p_coordinate_transform,
            IndentFileWriter p_file, eu.mihosoft.freerouting.library.Package p_package) throws java.io.IOException
    {
        Collection<eu.mihosoft.freerouting.board.Item> board_items =  p_board.get_items();
        boolean component_found = false;
        for (int i = 1; i <= p_board.components.count(); ++i)
        {
            eu.mihosoft.freerouting.board.Component curr_component =  p_board.components.get(i);
            if (curr_component.get_package() == p_package)
            {
                ...
                Iterator<eu.mihosoft.freerouting.board.Item> it = board_items.iterator();
                while (it.hasNext())
                {
    ...
    public static void write_component(BasicBoard p_board, IdentifierType p_identifier_type, CoordinateTransform p_coordinate_transform,
            IndentFileWriter p_file, eu.mihosoft.freerouting.board.Component p_component) throws java.io.IOException
    {
        p_file.new_line();
        p_file.write("(place ");
        ...
        for (eu.mihosoft.freerouting.board.Pin curr_pin : board_pins)
        {
            ...
            if (curr_pin.get_changed_to() != curr_pin)
            {
                ...
                if (curr_cmp != null)
                {
                    ...
                    p_file.write("-");
                    eu.mihosoft.freerouting.library.Package.Pin package_pin = curr_cmp.get_package().get_pin(curr_pin.get_index_in_package());
                ...
                if (swap_cmp != null)
                {
                    ...
                    p_file.write("-");
                    eu.mihosoft.freerouting.library.Package.Pin package_pin = swap_cmp.get_package().get_pin(swapped_with.get_index_in_package());
                    p_identifier_type.write(package_pin.name, p_file);
                }
                else
                {
    ...
    private static void write_net(int p_net_no, BasicBoard p_board, IdentifierType p_identifier_type, CoordinateTransform p_coordinate_transform,
            IndentFileWriter p_file) throws java.io.IOException
    {
        Collection<eu.mihosoft.freerouting.board.Item> net_items = p_board.get_connectable_items(p_net_no);
        boolean header_written = false;
        Iterator<eu.mihosoft.freerouting.board.Item> it = net_items.iterator();
        while (it.hasNext())
        {
            eu.mihosoft.freerouting.board.Item curr_item = it.next();
            if (curr_item.get_fixed_state() == eu.mihosoft.freerouting.board.FixedState.SYSTEM_FIXED)
            {
                continue;
            }
