...
public abstract class Shape
{
     ...
     */
    public static eu.mihosoft.freerouting.geometry.planar.Area transform_area_to_board(Collection<Shape> p_area, CoordinateTransform p_coordinate_transform)
    {
        ...
        eu.mihosoft.freerouting.geometry.planar.Shape boundary_shape = boundary.transform_to_board(p_coordinate_transform);
        eu.mihosoft.freerouting.geometry.planar.Area result;
        if (hole_count == 0)
        {
        ...
        else
        {
            ...
            result = new eu.mihosoft.freerouting.geometry.planar.PolylineArea(border, holes);
        }
        return result;
    }
     ...
     * the other shapes are holes of the area.
     */
    public static eu.mihosoft.freerouting.geometry.planar.Area transform_area_to_board_rel(Collection<Shape> p_area, CoordinateTransform p_coordinate_transform)
    {
        int hole_count = p_area.size() - 1;
        if (hole_count <= -1)
        {
        ...
        Iterator<Shape> it = p_area.iterator();
        Shape boundary = it.next();
        eu.mihosoft.freerouting.geometry.planar.Shape boundary_shape = boundary.transform_to_board_rel(p_coordinate_transform);
        eu.mihosoft.freerouting.geometry.planar.Area result;
        if (hole_count == 0)
        {
            result = boundary_shape;
