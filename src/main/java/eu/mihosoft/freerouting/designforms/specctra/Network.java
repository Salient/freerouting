...
public class Network extends ScopeKeyword
{
    ...
    public boolean read_scope(ReadScopeParameter p_par)
    {
        ...
        Collection<eu.mihosoft.freerouting.rules.ViaInfo> via_infos = new LinkedList<eu.mihosoft.freerouting.rules.ViaInfo>();
    ...
    public static void write_scope(WriteScopeParameter p_par) throws java.io.IOException
    {
        ...
        Collection<eu.mihosoft.freerouting.board.Pin> board_pins = p_par.board.get_pins();
    ...
    public static void write_via_infos(eu.mihosoft.freerouting.rules.BoardRules p_rules, IndentFileWriter p_file, IdentifierType p_identifier_type)
            throws java.io.IOException
    {
        for (int i = 0; i < p_rules.via_infos.count(); ++i)
        {
            eu.mihosoft.freerouting.rules.ViaInfo curr_via = p_rules.via_infos.get(i);
            p_file.start_scope();
            p_file.write("via ");
            p_file.new_line();
    ...
    public static void write_via_rules(eu.mihosoft.freerouting.rules.BoardRules p_rules, IndentFileWriter p_file, IdentifierType p_identifier_type)
            throws java.io.IOException
    {
        for (eu.mihosoft.freerouting.rules.ViaRule curr_rule : p_rules.via_rules)
        {
            p_file.start_scope();
    ...
    public static void write_net_class(eu.mihosoft.freerouting.rules.NetClass p_net_class, WriteScopeParameter p_par)
            throws java.io.IOException
    {
        p_par.file.start_scope();
    ...
    private static void write_circuit(eu.mihosoft.freerouting.rules.NetClass p_net_class, WriteScopeParameter p_par)
            throws java.io.IOException
    {
    ...
    static eu.mihosoft.freerouting.rules.ViaInfo read_via_info(Scanner p_scanner, eu.mihosoft.freerouting.board.BasicBoard p_board)
    {
        try
        {
            p_scanner.yybegin(SpecctraFileScanner.NAME);
            ...
            eu.mihosoft.freerouting.library.Padstack via_padstack = p_board.library.get_via_padstack(padstack_name);
    ...
    static Collection<String> read_via_rule(Scanner p_scanner, eu.mihosoft.freerouting.board.BasicBoard p_board)
    {
        try
        {
    ...
    private static void insert_via_infos(Collection<eu.mihosoft.freerouting.rules.ViaInfo> p_via_infos, RoutingBoard p_board, boolean p_attach_allowed)
    {
        if (p_via_infos.size() > 0)
        {
            for (eu.mihosoft.freerouting.rules.ViaInfo curr_info : p_via_infos)
            {
    ...
    private static void create_default_via_infos(eu.mihosoft.freerouting.board.BasicBoard p_board, eu.mihosoft.freerouting.rules.NetClass p_net_class, boolean p_attach_allowed)
    {
        int cl_class = p_net_class.default_item_clearance_classes.get(eu.mihosoft.freerouting.rules.DefaultItemClearanceClasses.ItemClass.VIA);
        ...
        {
            eu.mihosoft.freerouting.library.Padstack curr_padstack = p_board.library.get_via_padstack(i);
            ...
            eu.mihosoft.freerouting.rules.ViaInfo found_via_info =
                    new eu.mihosoft.freerouting.rules.ViaInfo(via_name, curr_padstack, cl_class, attach_allowed, p_board.rules);
            p_board.rules.via_infos.add(found_via_info);
        }
    }

    private static void insert_via_rules(Collection<Collection<String>> p_via_rules, eu.mihosoft.freerouting.board.BasicBoard p_board)
    {
    ...
    static boolean add_via_rule(Collection<String> p_name_list, eu.mihosoft.freerouting.board.BasicBoard p_board)
    {
        ...
        eu.mihosoft.freerouting.rules.ViaRule existing_rule = p_board.rules.get_via_rule(rule_name);
        eu.mihosoft.freerouting.rules.ViaRule curr_rule = new eu.mihosoft.freerouting.rules.ViaRule(rule_name);
        boolean rule_ok = true;
        while (it.hasNext())
        {
            eu.mihosoft.freerouting.rules.ViaInfo curr_via = p_board.rules.via_infos.get(it.next());
            if (curr_via != null)
            {
    ...
    static void insert_net_class(NetClass p_class, LayerStructure p_layer_structure, eu.mihosoft.freerouting.board.BasicBoard p_board, CoordinateTransform p_coordinate_transform,
                                 boolean p_via_at_smd_allowed)
    {
        eu.mihosoft.freerouting.rules.NetClass board_net_class = p_board.rules.append_net_class(p_class.name);
        ...
        {
            eu.mihosoft.freerouting.rules.ViaRule via_rule = p_board.rules.get_via_rule(p_class.via_rule);
            if (via_rule != null)
            {
        ...
        {
            Collection<eu.mihosoft.freerouting.rules.Net> curr_net_list = p_board.rules.nets.get(curr_net_name);
            for (eu.mihosoft.freerouting.rules.Net curr_net : curr_net_list)
            {
    ...
    static private void insert_class_pair_info(NetClass.ClassClass p_class_class, eu.mihosoft.freerouting.rules.NetClass p_first_class, eu.mihosoft.freerouting.rules.NetClass p_second_class,
                                               eu.mihosoft.freerouting.board.BasicBoard p_board, CoordinateTransform p_coordinate_transform)
    {
    ...
    static private void add_mixed_clearance_rule(eu.mihosoft.freerouting.rules.ClearanceMatrix p_clearance_matrix, eu.mihosoft.freerouting.rules.NetClass p_first_class,
                                                 eu.mihosoft.freerouting.rules.NetClass p_second_class, Rule.ClearanceRule p_clearance_rule, int p_layer_no,
    ...
    static private void create_default_clearance_classes(eu.mihosoft.freerouting.rules.NetClass p_net_class,
                                                         eu.mihosoft.freerouting.rules.ClearanceMatrix p_clearance_matrix)
    {
    ...
    private static void create_via_rule(Collection<String> p_use_via, eu.mihosoft.freerouting.rules.NetClass p_net_class, eu.mihosoft.freerouting.board.BasicBoard p_board, boolean p_attach_allowed)
    {
        eu.mihosoft.freerouting.rules.ViaRule new_via_rule = new eu.mihosoft.freerouting.rules.ViaRule(p_net_class.get_name());
        int default_via_cl_class = p_net_class.default_item_clearance_classes.get(eu.mihosoft.freerouting.rules.DefaultItemClearanceClasses.ItemClass.VIA);
        for (String curr_via_name : p_use_via)
        {
            for (int i = 0; i < p_board.rules.via_infos.count(); ++i)
            {
                eu.mihosoft.freerouting.rules.ViaInfo curr_via_info = p_board.rules.via_infos.get(i);
    ...
    private static void create_active_trace_layers(Collection<String> p_use_layer, LayerStructure p_layer_structure, eu.mihosoft.freerouting.rules.NetClass p_net_class)
    {
    ...
    private static void add_clearance_rule(eu.mihosoft.freerouting.rules.ClearanceMatrix p_clearance_matrix, eu.mihosoft.freerouting.rules.NetClass p_net_class,
     ...
     */
    static private int get_clearance_class(eu.mihosoft.freerouting.rules.ClearanceMatrix p_clearance_matrix,
                                           eu.mihosoft.freerouting.rules.NetClass p_net_class, String p_item_class_name)
    {
    ...
    private static boolean insert_logical_parts(ReadScopeParameter p_par)
    {
        ...
        {
            eu.mihosoft.freerouting.library.LogicalPart curr_logical_part = routing_board.library.logical_parts.get(next_mapping.name);
            {
            ...
            {
                eu.mihosoft.freerouting.board.Component curr_component = routing_board.components.get(curr_cmp_name);
    ...
    private static eu.mihosoft.freerouting.library.Package search_lib_package(String p_part_name,
                                                                              java.util.Collection<PartLibrary.LogicalPartMapping> p_logical_part_mappings, eu.mihosoft.freerouting.board.BasicBoard p_board)
    {
            ...
            ReadScopeParameter p_par)
    {
        ...
        eu.mihosoft.freerouting.board.Component new_component = routing_board.components.add(p_location.name, component_location,
        ...
        {
            fixed_state = eu.mihosoft.freerouting.board.FixedState.SYSTEM_FIXED;
        }
        else
        {
            fixed_state = eu.mihosoft.freerouting.board.FixedState.UNFIXED;
        }
        eu.mihosoft.freerouting.library.Package curr_package = new_component.get_package();
        for (int i = 0; i < curr_package.pin_count(); ++i)
        {
            eu.mihosoft.freerouting.library.Package.Pin curr_pin = curr_package.get_pin(i);
            ...
            eu.mihosoft.freerouting.rules.NetClass net_class;
            eu.mihosoft.freerouting.rules.Net board_net;
            ...
            if (clearance_class < 0)
            {
                if (curr_padstack.from_layer() == curr_padstack.to_layer())
                {
                    clearance_class = net_class.default_item_clearance_classes.get(eu.mihosoft.freerouting.rules.DefaultItemClearanceClasses.ItemClass.SMD);
                }
                else
                {
                    clearance_class = net_class.default_item_clearance_classes.get(eu.mihosoft.freerouting.rules.DefaultItemClearanceClasses.ItemClass.PIN);
                }
            }
        ...
        for (int k = 0; k <= 2; ++k)
        {
            eu.mihosoft.freerouting.library.Package.Keepout[] keepout_arr;
            ...
            {
                eu.mihosoft.freerouting.library.Package.Keepout curr_keepout = keepout_arr[i];
                ...
                int clearance_class =
                        routing_board.rules.get_default_net_class().default_item_clearance_classes.get(eu.mihosoft.freerouting.rules.DefaultItemClearanceClasses.ItemClass.AREA);
